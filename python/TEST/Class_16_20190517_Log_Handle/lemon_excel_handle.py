...
class HandleExecl(object):
    ...
    def write_result(self,row,actual,result):
        ...
        """
        if isinstance(row, int) and (2 <= row <= self.ws.max_row):
            self.ws.cell(row=row,column=HandleExecl.config("excel","actual_col"),value=actual)
            self.ws.cell(row=row, column=HandleExecl.config("excel","result_col"), value=result)
