...
from TEST.Class_16_20190513_Parser_config.lemon_04_parser_config_4 import HandleConfing
...
class TestMulti(unittest.TestCase):  #继承unittest.TeatCase类
    ...
    case_obj = HandleExecl(config("file path","cases_path"))
    ...
    def setUpClass(cls):
        ...
        cls.file_name = cls.config("file path","log_path")
    ...
    def test_negatives_multi(self,data_namedtuple):  #测试用例方法名必须以test开头
        ...
        #将实际结果写入excel
        #self.ws.cell(row=case_id+1,actual=real_result,value = real_result)
        run_success_msg = TestMulti.config("msg","sussess_result")
        run_fail_msg = TestMulti.config("msg", "fail_result")
        try:
            ...
            self.case_obj.write_result(row=case_id+1,actual=real_result,result="Fail") #将执行失败的用例fail
